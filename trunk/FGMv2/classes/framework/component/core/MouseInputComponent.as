	...
	public class MouseInputComponent extends Component {
		...
		private function initVar():void {
			...
			if(_testApi){
				...
				FlashGameMaker.AddChild(_mouseTest);
				_displayTest.mouseEnabled = false;
				FlashGameMaker.AddChild(_displayTest);
		...
		public function initListener():void {
			...
			FlashGameMaker.stage.addEventListener(MouseEvent.MOUSE_DOWN, onMouseDown, true, 0, true);
		...
		public function removeListener():void {
			...
			FlashGameMaker.stage.removeEventListener(MouseEvent.MOUSE_DOWN, onMouseDown,true);
		...
		private function dispatch($callback:String):void {
			...
			_mouseGlobal = new Point(FlashGameMaker.clip.mouseX,FlashGameMaker.clip.mouseY);
				...
				if($callback == "onMouseMove" && !drag){
					if(object.param.hasOwnProperty("onMouseRollOut") && rollOver==object.component && !hitTest){
						...
						if(rollOver && checkPosition(rollOver)){
							if(FlashGameMaker.IndexOf(rollOver)>FlashGameMaker.IndexOf(object.component)) return;
