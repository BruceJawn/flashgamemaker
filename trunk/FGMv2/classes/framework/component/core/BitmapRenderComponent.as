	...
	*/
	public class BitmapRenderComponent extends Component {
		...
		private function _initVar($prop:Object):void {
			...
			_bitmapData = new BitmapData(FlashGameMaker.stage.fullScreenWidth, FlashGameMaker.stage.fullScreenHeight,true,0);
			//_bitmapCache = new BitmapData(FlashGameMaker.stage.fullScreenWidth, FlashGameMaker.stage.fullScreenHeight,true,0);
			_bitmap = new Bitmap(_bitmapData);
			_timeline = new Dictionary(true);
			FlashGameMaker.AddChild(_bitmap,this);//Properties of BitmapRender will influe Display
			FlashGameMaker.AddChild(this,null,0);
		...
		private function onTick():void {
			...
			if(this.mask!=null){
				...
				area = FlashGameMaker.clip.getBounds(FlashGameMaker.clip);
			}
			scrollView();
			updateView();
			_bitmapData.lock();
			...
			fastSort(_drawableComponent, sortDepths);
			if(_testPerformance){
		...
		private function scrollView():void {
			if(_scrollTarget){
				if(_scrollTarget.x<10){
					_position.x=-10;
				}else if(_scrollTarget.x>FlashGameMaker.width-100){
					_position.x=10;
				...
				if(_scrollTarget.y<10){
					_position.y=-10;
				}else if(_scrollTarget.y>FlashGameMaker.height-100){
					_position.y=10;
