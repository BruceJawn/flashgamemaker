	...
	public class LayoutUtil {
		...
		public static function Align( $subject:DisplayObject, $subjectAlign:String, $reference:DisplayObject=null, $refAlign:String=null, $offset:Point=null, $addAsChild:Boolean=false, $round:Boolean=true, $checkBorder:Boolean=false  ):void {
			...
			if(!$reference || $reference.width ==0 || $reference.height==0){
				...
				rect.graphics.drawRect(0,0,FlashGameMaker.width, FlashGameMaker.height);
		...
		public static function getStage():DisplayObject {
			...
			var r:Rectangle = new Rectangle(0,0,FlashGameMaker.width, FlashGameMaker.height);
		...
		public static function drawDebugGuide( $subject:DisplayObject, $color:int=0xFF0000, $reference:DisplayObjectContainer=null ):void {
			var ref:DisplayObjectContainer = $reference ? $reference: FlashGameMaker.stage;
		...
		public static function describeDisplayObject( $disp:DisplayObject, $indent:String="\t" ):void{
			trace($indent
				...
				+ ", x:"+$disp.x
				+ ", y:"+$disp.y
				+ " bounds:"+$disp.getBounds(FlashGameMaker.stage).toString()
			);
		...
		public static function CheckBorder($reference:DisplayObject):void{
			if($reference.x<0){		
				$reference.x=0;
			}else if($reference.x+$reference.width>FlashGameMaker.stage.stageWidth){	
				$reference.x=FlashGameMaker.stage.stageWidth-$reference.width-1;
			}
			if($reference.y<0){		
				$reference.y=0;
			}else if($reference.y+$reference.height>FlashGameMaker.stage.stageHeight){	
				$reference.y=FlashGameMaker.stage.stageHeight-$reference.height-1;
			}
		}
	}
}
