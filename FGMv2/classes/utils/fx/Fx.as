	...
	public class Fx {
		...
		public static function ShowModal($clip:MovieClip,$color:uint=0 ):Sprite {
			if(!_modal || !$clip.contains(_modal)){
				...
				var bounds:Rectangle = FlashGameMaker.clip.getBounds(null);
				...
				FlashGameMaker.clip.setChildIndex(_modal, FlashGameMaker.clip.numChildren-1);
			}
			return _modal;
		}
		//------ Hide Modal ------------------------------------
		...
		public static function Night($clip:DisplayObject, $duration:Number=1000):void {
			...
			timer.addEventListener(TimerEvent.TIMER, PassParameters.AddArguments(onTimer, [$clip]));
			timer.start();
		}
		//------ Day ------------------------------------
		...
		public static function DisappearingText($position:Point, $text:String, $color:uint=0xFFFFFF, $distance:Point=null,$duration:Number=5):void {
			...
			FlashGameMaker.clip.addChild(tf);
			tf.x = $position.x;
			tf.y = $position.y;
