	...
	*/
	public class ScrollingBitmapComponent extends GraphicComponent {
		...
		protected override function onGraphicLoadingComplete($graphic:DisplayObject):void {
			...
			_bitmap.bitmapData = new BitmapData(_graphic.width,_graphic.height );
		...
		public function scrollH():void {
			if(_rectangle.x+_rectangle.width+_speed.x<_graphic.width){
				_rectangle.x+=_speed.x;
			}else if(_loop){
				if(_rectangle.x+_speed.x<_graphic.width){
					_rectangle.x+=_speed.x;
					var dist:Number = _rectangle.x+_rectangle.width+_speed.x - _graphic.width;
