...
package framework.add.architecture.component{
	...
	*/
	public class GamePadComponent extends SimpleGraphicComponent {
		...
		public var _gamePad:Object=null;
		...
		//------ On Key Fire ------------------------------------
		public function onKeyFire($gamePad:Object):void {
			_gamePad = $gamePad;
			updateGamePad();
			dispatch("onKeyFire");
		}
		...
		private function updateGamePad():void {
			var targetAngle:Number=Math.atan2(_gamePad.targetX,_gamePad.targetY);
			if (_isCircle&&_gamePad.anyDirection.isDown) {
				_gamePad.targetX=Math.sin(targetAngle);
				_gamePad.targetY=Math.cos(targetAngle);
			}
			_ball.x=_gamePad.x*_ballStep;
			_ball.y=_gamePad.y*_ballStep;
			if (_button1.alpha!=0) {
				_button1.alpha=_gamePad.fire1.isDown?1:0.5;
			}
			if (_button2.alpha!=0) {
				_button2.alpha=_gamePad.fire2.isDown?1:0.5;
			}
			if (_button3.alpha!=0) {
				_button3.alpha=_gamePad.fire3.isDown?1:0.5;
			}
			if (_button4.alpha!=0) {
				_button4.alpha=_gamePad.fire4.isDown?1:0.5;
			}
			if (_up.alpha!=0) {
				_up.alpha=_gamePad.up.isDown?1:0.5;
			}
			if (_down.alpha!=0) {
				_down.alpha=_gamePad.down.isDown?1:0.5;
			}
			if (_left.alpha!=0) {
				_left.alpha=_gamePad.left.isDown?1:0.5;
			}
			if (_right.alpha!=0) {
				_right.alpha=_gamePad.right.isDown?1:0.5;
			}
		}
		//------ DisplayButton ------------------------------------
